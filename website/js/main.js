...
// Destination Explorer Switcher
function initDestinationExplorer() {
    const destBtns = document.querySelectorAll('.dest-nav-btn');
    ...
    destBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const targetPanel = document.getElementById('dest-panel-' + targetCountry);
            if (targetPanel) targetPanel.classList.add('active');
        });
    });
}

// Partner Lenders Interactive Filter Engine
function initLenderFilters() {
    const filterBtns = document.querySelectorAll('.lender-filter-btn');
    const lenderCards = document.querySelectorAll('.lender-info-card');
    if (!filterBtns.length || !lenderCards.length) return;
    ...
    filterBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const filterValue = this.getAttribute('data-filter-val');

            const groupBtns = document.querySelectorAll(`.lender-filter-btn[data-filter-group="${filterGroup}"]`);
            ...
            lenderCards.forEach(card => {
                ...
                const cardLoanTypes = (card.getAttribute('data-loan-types') || '').split(',');
                const cardDests = (card.getAttribute('data-dest') || '').split(',');

                const matchesType = activeFilters.type === 'all' || cardType === activeFilters.type;
                const matchesLoanType = activeFilters.loanType === 'all' || cardLoanTypes.includes(activeFilters.loanType);
                const matchesDest = activeFilters.dest === 'all' || cardDests.includes(activeFilters.dest);

                if (matchesType && matchesLoanType && matchesDest) {
                    card.style.display = '';
                } else {
